...
// Publish to /cmd_vel, /move_base_simple/goal, and /move_base/cancel

/* TODO
1. Change default-speed to force-speed
2. What if see both counter and plant? (ignore for now)
*/
...
void objectPositionReceived(const darknet_ros_msgs::BoundingBoxes::ConstPtr& object) {  //output direction, move_state
    if((object->bounding_boxes[0].Class == "counter") && (status == "goal_counter")){
        ...
        if((width>1000) && (move_state == 1)) move_state = 2; //at closest point

        if(center < 560) direction = 0; //need to turn left
        else if(center > 870) direction = 2; //need to turn right
        else direction = 1;
    }

    else if((object->bounding_boxes[0].Class == "plant") && (status == "goal_gate")){
        if(object->bounding_boxes[1].Class == "plant"){ //see 2 plants, bring to center (gate)
            ...
            center = (object->bounding_boxes[0].xmin + object->bounding_boxes[0].xmax)/2.0;
            width = object->bounding_boxes[0].xmax - object->bounding_boxes[0].xmin;
            if((width>700) && (move_state == 1)) move_state = 2; //at closest point
        }
    ...
    else if((object->bounding_boxes[0].Class == "suitcase") && (status == "goal_suitcase")){
        center = (object->bounding_boxes[0].xmin + object->bounding_boxes[0].xmax)/2.0;
        width = object->bounding_boxes[0].xmax - object->bounding_boxes[0].xmin;
...
int main(int argc, char **argv) {
    ...
    while(ros::ok()){
        ...
        nh.param("distance", distance, float(5)); //default distance is 5
                                                  //can be updated by "rosparam set distance xx"
